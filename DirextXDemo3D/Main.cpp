...
#
	...
	// -- Constant Buffer to Descriptor and Upload Heap -- // 
	for (int i = 0; i < frameBufferCount; ++i) {
	...
	CD3DX12_RESOURCE_DESC bufferSizeDesc = CD3DX12_RESOURCE_DESC::Buffer(1024 * 64);
	...
	for (int i = 0; i < frameBufferCount; ++i) {
		...
		ZeroMemory(&cbColourMultiplierData, sizeof(cbColourMultiplierData));

		CD3DX12_RANGE readRange(0, 0); //Not reading from the CPU
		hr = constantBufferUploadHeap[i]->Map(0, &readRange, reinterpret_cast<void**>(&cbColourMultiplierGPUAddress[i]));
		memcpy(cbColourMultiplierGPUAddress[i], &cbColourMultiplierData, sizeof(cbColourMultiplierData));
	}
...
void Update() {
	static float rIncrement = 0.00004f;
	static float gIncrement = 0.00005f;
	static float bIncrement = 0.00006f;

	cbColourMultiplierData.colourMultiplier.x += rIncrement;
	cbColourMultiplierData.colourMultiplier.y += gIncrement;
	cbColourMultiplierData.colourMultiplier.z += bIncrement;

	if (cbColourMultiplierData.colourMultiplier.x >= 1.0 || cbColourMultiplierData.colourMultiplier.x <= 0.0) {
		cbColourMultiplierData.colourMultiplier.x = cbColourMultiplierData.colourMultiplier.x >= 1.0 ? 1.0 : 0.0;
		rIncrement = -rIncrement;
	}
	if (cbColourMultiplierData.colourMultiplier.y >= 1.0 || cbColourMultiplierData.colourMultiplier.y <= 0.0) {
		cbColourMultiplierData.colourMultiplier.y = cbColourMultiplierData.colourMultiplier.y >= 1.0 ? 1.0 : 0.0;
	...
	memcpy(cbColourMultiplierGPUAddress[frameIndex], &cbColourMultiplierData, sizeof(cbColourMultiplierData));
}
